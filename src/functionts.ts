...
function moveSlide(presentation: Presentation, oldslideIndex: number, newslideIndex: number): Presentation {
    const selectedSlide = presentation.slideList[oldslideIndex]
    if(!selectedSlide){
        console.error('Выделенный слайд не найден!')
    }

    const selectedSlideIndex = presentation.slideList.indexOf(selectedSlide)+ oldslideIndex
    
    let slides: Array<Slide> = []
    for(let i = 0; i < presentation.slideList.length; i++){
        if(i !== presentation.slideList.indexOf(selectedSlide)){
            slides.push(presentation.slideList[i])
        }
    }
    
    if (selectedSlideIndex > presentation.slideList.length - 1) {
